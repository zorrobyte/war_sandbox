...
if (typeName _a == "Object") then
{
	...
	{
		hint "Error: Please create a ""Group Link 4: System"" module to initialize the ""Group Link 4: Defend"" module.";
	}
	else
	{
		...
		if (count _b > 0) then
		{
			...
			while { (_c < count _b) } do
			{
				...
				if (group _d in (GL4_Defend select 0) ) then
				{
					hint format ["Note: %1 was synchronized twice with the ""Group Link 4: Defend"" module.", group _d];
				}
				else
				{
					GL4_Defend set [0, (GL4_Defend select 0) + [group _d] ];
...
else
{
	...
	while { (_d < 5) } do
	{
		...
		else
		{
			...
			GL4_Defend set [0, (GL4_Defend select 0) + [_c] ];
