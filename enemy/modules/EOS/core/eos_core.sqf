...
_debug=if (count _settings > 5) then {_settings select 5} else {true};
...
if (!(getmarkercolor _mkr == "colorblack")) then {	
	null = [_mkr,[_aGrps,_aSize],[_bGrps,_bSize],[_cGrps,_cSize],[_dGrps,_eGrps,_fGrps,_fSize],_settings,true] execVM "enemy\modules\EOS\core\eos_core.sqf";
