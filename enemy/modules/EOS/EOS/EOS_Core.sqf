...
if (isNil("Z_Debug")) then {Z_Debug=false};
_heightLimit=false;
_r=_mkrX * 0.8;
		...
		if (_cache) then {
				...
				_units=_eosActivated getvariable _cacheGrp;	
						...
						_aMin=_aSize select 0;
							if (Z_Debug)then{player sidechat format ["ID:%1,restore - %2",_cacheGrp,_units];};
							};
								if (_aMin > 0) then {
			...
			_pos = [_pos,0,125,5,1,20,0] call BIS_fnc_findSafePos;
				_eGroup=[_pos,_bSize,_faction,_side] call EOS_spawnStatic;
					_eGrp=_eGrp+[_eGroup];sleep 0.1;
							if (Z_Debug) then {hint "Spawning static Vehicles";0= [_mkr,(count _eGrp),"Static",(getpos leader (_eGroup select 2))] call EOS_debug};
		};	
	...
	_eGrpB=[];
		...
		{	
			...
			_eType=true;_eGrpsB=0;
						...
						0=[_mkr,_eSize,_faction,_eType,_n,_eosActivated,_side] execVM "enemy\modules\EOS\SpawnUnits\Spawn_Aircraft.sqf";
						_eGrpB=_eGrpB+[_n];
							if (Z_Debug) then {hint "Spawning HELI TRANSPORT";};
							_n=_n+1;sleep 0.1;
...
while {_eosAct} do
	...
	if (!triggeractivated _eosActivated || getmarkercolor _mkr == "colorblack") exitwith 
		{
		if (Z_Debug) then {if (!(getmarkercolor _mkr == "colorblack")) then {hint "Restarting Zone AND deleting units";}else{hint "EOS zone deactivated";};};		
...
if (Z_Debug) then {player sidechat format ["ID:c%1",_cGrps];};};
...
if (Z_Debug) then {player sidechat format ["ID:c%1",_dGrps];};};

// CACHE PATROL INFANTRY					
	if (count _bGrp > 0) then 
				{		_n=0;					
						{	_n=_n+1;_units={alive _x} count units _x;_cacheGrp=format ["PA%1",_n];
	if (Z_Debug) then{player sidechat format ["ID:%1,cache - %2",_cacheGrp,_units];};
...
_eosAct=false;
if (Z_Debug) then {hint "Zone Cached";};
};
				...
				while {triggeractivated _eosActivated AND !(getmarkercolor _mkr == "colorblack")} do 
						{
							if (!triggeractivated _clear) then
							{
								...
								if (Z_Debug) then {hint "Zone Lost";};
										}else{
...
if (!(getmarkercolor _mkr == "colorblack")) then {	
	null = [_mkr,[_aGrps,_aSize],[_bGrps,_bSize],[_cGrps,_cSize],[_dGrps,_dSize],[_eGrps,_eGrpsB,_eSize],_settings,true] execVM "enemy\modules\EOS\EOS\EOS_Core.sqf";
