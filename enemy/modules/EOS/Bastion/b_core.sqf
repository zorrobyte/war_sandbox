...
waituntil {triggeractivated _bastTaken};
		...
		_n=_n+1;		
						...
						_pos = [_pos,0,20,5,1,20,0] call BIS_fnc_findSafePos;
							...
							_aGroup=_aGroup+[_grp];	
								sleep 0.1;
									if (Z_Debug) then {hint "Spawning patrol";
	...
	while {(count _lv) < _LVehGroups} do 
		{		
			...
			_lv=_lv+[_bGroup];		
			sleep 0.1;
				if (Z_Debug) then {player sidechat format ["Light Vehicle:%1 - r%2",(count _lv),_LVehGroups];
...
//SPAWN ARMOURED VEHICLES
	...
	_TR=[];
		...
		{	
			...
			0=[_mkr,_CHgroupSize,_faction,_CHType,_n,_bastActive,_side] execVM "enemy\modules\EOS\SpawnUnits\Spawn_Aircraft.sqf";
			_TR=_TR+[_n];
