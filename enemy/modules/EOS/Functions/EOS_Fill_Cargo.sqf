...
private ["_cargo","_emptySeats","_vehicle","_debug","_filled","_unit","_grp","_grpSize","_side"];
...
// FILL EMPTY SEATS		
	...
	if (Z_Debug) then {hint format ["%1",_emptySeats];};
...
// LIMIT SEATS TO FILL TO GROUP SIZE		
			if (_emptySeats>_grpSize) then {_emptySeats=_grpSize};
						...
						if (Z_Debug) then {hint format ["Seats Filled : %1",_emptySeats];};	
